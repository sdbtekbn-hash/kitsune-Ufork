...
mod ffi {
    ...
    unsafe extern "C++" {
        ...
        fn is_deny_target_rs(uid: i32, process: &str, max_len: i32) -> bool;
        fn initialize_denylist_rs();
        fn init_nethunter_mode_rs();
        fn enable_nethunter_mode_rs();
        fn disable_nethunter_mode_rs();
        
        // ReZygisk Integration
        fn init_solist_hiding_rs();
        fn init_seccomp_hiding_rs();
        fn init_ptrace_hiding_rs();
        fn solist_reset_counters_rs(load: usize, unload: usize);
        unsafe fn zygisk_cleanup_with_jni_rs(env: *mut u8);
        unsafe fn register_plt_hook_rs(symbol: *mut u8, backup: *mut *mut u8);
        unsafe fn register_jni_hook_rs(clz: &str, method: *mut u8);
        fn restore_plt_hooks_rs();
        unsafe fn restore_jni_hooks_rs(env: *mut u8);
        fn reset_module_counters_rs();
        fn send_seccomp_event_rs();
        fn trace_zygote_rs(pid: i32) -> bool;
        fn cleanup_ptrace_rs();
        fn is_ptrace_active_rs() -> bool;
        fn restore_zygisk_prop_rs();
        fn switch_mnt_ns_rs(pid: i32) -> i32;
        fn app_request_rs(req: &SuAppRequest) -> i32;
        fn app_notify_rs(req: &SuAppRequest, policy: SuPolicy);
        fn app_log_rs(req: &SuAppRequest, policy: SuPolicy, notify: bool);
        fn exec_root_shell_rs(client: i32, pid: i32, req: &mut SuRequest, mode: MntNsMode);
        fn get_manager_for_cxx_rs(user: i32, pkg: &mut String, install: bool) -> i32;
        fn load_modules_su_rs();
        fn parse_mount_info_rs_wrapper(pid: &str) -> Vec<String>;
        fn setup_logfile_rs();
        fn android_logging_rs();
        fn restorecon_rs();
        fn setfilecon_rs(path: &str, con: &str) -> bool;
        fn find_preinit_device_rs() -> String;
    }
}
